...
function gck_split_attrs_color_size( array $custom_attrs ) : array {
    ...
    foreach ( $custom_attrs as $key => $attr ) {
        ...
        $is_color = ( strpos($hay, 'parte') !== false || strpos($hay, 'boja') !== false || strpos($hay, 'barva') !== false || strpos($hay, 'farb') !== false || strpos($hay, 'culoare') !== false || strpos($hay, 'color') !== false || strpos($hay, 'colour') !== false || strpos($hay, 'colore') !== false || strpos($hay, 'kolor') !== false || strpos($hay, 'szín') !== false || strpos($hay, 'szin') !== false || strpos($hay, 'ρώμ') !== false || strpos($hay, 'цвят') !== false );
...
function gck_ro_majice_phrase( int $n, bool $free = false, string $type = 'majica' ) : string {
    if ( $type === 'steznica' ) {
        if ( $n === 1 ) { $noun = 'orteză'; $adj = 'gratuită'; }
        else { $noun = 'orteze'; $adj = 'gratuite'; }
        return $free ? ( $noun . ' ' . $adj ) : $noun;
    }
    if ( $type === 'bokserica' ) {
        if ( $n === 1 ) {
            $noun = 'boxer'; $adj = 'gratuit';
        } else {
            $noun = 'boxeri'; $adj = 'gratuiți';
        }
        return $free ? ( $noun . ' ' . $adj ) : $noun;
    }
    if ( $type === 'carapa' ) {
        if ( $n === 1 ) {
            $noun = 'pereche de șosete'; $adj = 'gratuită';
        } else {
            $noun = 'perechi de șosete'; $adj = 'gratuite';
        }
        return $free ? ( $noun . ' ' . $adj ) : $noun;
    }
    if ( $n === 1 ) {
        $noun = 'tricou'; $adj = 'gratuit';
    } else {
        $noun = 'tricouri'; $adj = 'gratuite';
    }
    return $free ? ( $noun . ' ' . $adj ) : $noun;
}
...
function gck_render_bundle_selector() {
    ...
    if (
        ...
        || ( stripos( (string) $product->get_slug(), 'soset' ) !== false )
        || ( stripos( (string) $product->get_name(), 'șoset' ) !== false )
        || ( stripos( (string) $product->get_name(), 'soset' ) !== false )
    ) {
    ...
    //  - orto-ortopas / orto-kidsnest : single "Mărime" attribute, no colour (size selector only).
    ...
    if ( has_term( array( 'orto-cloath' ), 'product_cat', $product_id ) ) {
        $gck_offer_subs = array( 'Set pentru baie', 'Pachet pentru toată casa', 'Pachet pentru familie și prieteni' );
    } else {
    ...
    // extended to 3 garment groups (4 tricou + 1 boxeri + 1 șosetă). Gated
    // strictly to this category. This site's save/sync collapses product
    // attributes to a single Culoare/Mărime, so the 3 groups' colour/size options
  ...
  margin-right:6px;
    ...
    <?php
    // SHGIFTS: the șosetă size labels ("39-43" …) are wider than shirt/boxer sizes,
    ...
    <?php if ( $show_countdown ) : ?>
        ...
        <div class="gck-countdown" id="gck-countdown"
            ...
            <div class="gck-countdown__body">
                Oferta este valabilă doar azi — încă <span class="gck-countdown__timer" aria-live="polite">--:--</span>.<?php
                if ( $countdown_stock > 0 ) :
                    ?> Au mai rămas doar <strong><?php echo esc_html( $countdown_stock ); ?> bucăți</strong>.<?php
                endif; ?> <strong>Grăbește-te înainte să se termine!</strong>
            </div>
        </div>
        <script>
          (function(){
    ...
    <style>
      ...
      /* lokalizirane barve RO */
      #bundle-selector .swatch-circle.color-albastru { background: #b7c4e6 !important; }
      #bundle-selector .swatch-circle.color-roz { background: #e6a8b8 !important; }
      #bundle-selector .swatch-circle.color-gri { background: #c9c2ba !important; }
      #bundle-selector .swatch-circle.color-verde { background: #b3bf9a !important; }
      #bundle-selector .swatch-circle.color-mov { background: #b48fc4 !important; }
      #bundle-selector .swatch-circle.color-bleumarin { background: #2b3a63 !important; }
    ...
    <style>
      ...
      /* lokalizirane barve RO */
      #bundle-selector .swatch-circle.color-negru { background: #17161a !important; }
      #bundle-selector .swatch-circle.color-gri-inchis { background: #4a4a4f !important; }
      #bundle-selector .swatch-circle.color-roz { background: #e28ba4 !important; }
    ...
    <div id="bundle-selector" class="bundle-box<?php echo $gck_single_size ? ' is-single-size' : ''; ?><?php echo $gck_no_attrs ? ' is-no-attrs' : ''; ?>" data-split-garments="<?php echo $gck_split_garments ? '1' : '0'; ?>">
        ...
        foreach ( $offers as $offer_id => $data ) :
            ...
            // SHGIFTS: real piece count is 6 (4 tricou + 1 boxeri + 1 șosetă),
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id ) 
                ...
                <?php if ( $show_price_highlights ) : ?>
                    ...
                    <span class="gck-per-chip">
                        <?php if ( $per_regular > (float) $data['per'] ) : ?>
                            <span class="gck-per-old"><?php echo number_format( $per_regular, 2 ); ?> lei</span>
                        <?php endif; ?>
                        <span class="gck-per-new"><?php echo number_format( $gck_per_new, 2 ); ?> lei / buc</span>
                    </span>
                ...
                <?php else : ?>
                    — <span class="bundle-option-title"><?php echo number_format( $gck_per_new, 2 ); ?> lei / buc</span>
                <?php endif; ?>
...
<div class="bundle-total-line">
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 2 ); ?> lei</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    // attributes — "Culoare tricou"/"Mărime tricou", "Culoare boxeri"/
                    // "Mărime boxeri", "Culoare șosetă"/"Mărime șosetă" — grouped by the
                    // garment word via the same helper SHBOX uses ($attr_groups).
                    ...
                    if ( $gck_split_garments ) {
                        $gck_passes = array(
                            array( 'group' => 0, 'label' => 'Alege ' . (int) $pairs . ' ' . gck_ro_majice_phrase( $pairs, false, 'majica' ), 'gratis' => false ),
                            array( 'group' => 1, 'label' => 'Alege încă ' . (int) $pairs . ' ' . gck_ro_majice_phrase( $pairs, true, 'bokserica' ), 'gratis' => true ),
                        );
                    } else {
 ...
 * total: tricou 1-4 (colour+size), boxeri (colour+size), șosetă (colour+size).
 * Each line is labelled by inspecting its field keys so the cart / order clearly
 * shows which garment was picked, e.g. "Tricou: Negru - L", "Boxeri: Albastru - M",
 * "Șosete: Negru - 39-43".
 */
function gck_build_shgifts_lines( array $pairs_data ) : array {
    ...
    foreach ( $pairs_data as $attrs ) {
        ...
        foreach ( array_keys( $attrs ) as $k ) {
            ...
            if ( strpos( $kl, 'majic' ) !== false )    { $label = 'Tricou'; break; }
            if ( strpos( $kl, 'bokseric' ) !== false ) { $label = 'Boxeri'; break; }
            if ( strpos( $kl, 'carap' ) !== false )    { $label = 'Șosete'; break; }
        }
...
function gck_add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
    ...
    // SHGIFTS (orto-majica-darila): 6 items (4 tricou + 1 boxeri + 1 șosetă),
...
function gck_shgifts_free_gifts_notice() {
    ...
    // Gifts shown: 1x boxeri + 5x pereche șosete. Images + crossed-out reference
    ...
    $gifts = array(
        array( 'label' => '1x boxeri', 'img' => $boxer_img, 'price' => '99,99 lei' ),
    );
    for ( $i = 0; $i < 5; $i++ ) {
        $gifts[] = array( 'label' => '1x pereche șosete', 'img' => $socks_img, 'price' => '64,99 lei' );
    }
    ?>
    <style>
      .gck-fg{
    ...
    <div class="gck-fg">
        <div class="gck-fg__head">6 produse GRATUITE la fiecare comandă</div>
