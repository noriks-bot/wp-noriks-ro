...
<!-- suport section -->
	...
	<?php
	// ===== Klaviyo newsletter =====
	// TODO(RO): replace with the Romanian footer list ID (Klaviyo -> Lists
	// & Segments -> Settings -> List ID). Left as placeholder so signups are
	// NOT sent to another market's list until the RO list is set.
	$nf_klaviyo_list = 'WM5Sbs';
	...
	if ( ! $nf_legal ) {
		$nf_legal = 'NORIKS BRAND. Toate drepturile rezervate. Prețurile includ TVA. Imaginile au caracter ilustrativ.';
	}
	?>
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Alătură-te familiei NORIKS</h2>
					<p class="nf-signup-sub">Oferte exclusive și anunțuri despre produse noi — află primul.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						<label class="nf-field-label" for="nf-email">E-mailul tău *</label>
						<div class="nf-field-row">
							<input type="email" id="nf-email" name="email" class="nf-input" placeholder="Adresa ta de e-mail" required autocomplete="email">
							<button type="submit" class="nf-btn nf-btn-light">Abonează-te</button>
						</div>
						<p class="nf-signup-note">
							Prin abonare ești de acord să primești e-mailuri. Te poți dezabona oricând.
						...
						<div class="nf-link-col">
							...
							if ( $nf_col2_l ) :
								foreach ( $nf_col2_l as $item ) :
									if ( false !== strpos( (string) $item['link'], '/instructiuni' ) ) { $nf_has_upute = true; }
									?>
								<a href="<?php echo esc_url( $item['link'] ); ?>"><?php echo esc_html( $item['text'] ); ?></a>
							<?php endforeach; endif; ?>
							<?php if ( ! $nf_has_upute ) : ?>
								<a href="/ro/instructiuni/">Instrucțiuni de utilizare</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/ro.svg" alt="" class="nf-flag">
					România (RO)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Toate drepturile rezervate.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Introdu o adresă de e-mail validă.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'Newsletterul nu este încă conectat (lipsește Klaviyo List ID).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			msg.textContent = 'Se procesează…'; msg.className = 'nf-signup-msg';
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Mulțumim! Verifică-ți e-mailul pentru confirmare.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Ceva nu a mers bine, încearcă din nou.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Ceva nu a mers bine, încearcă din nou.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
