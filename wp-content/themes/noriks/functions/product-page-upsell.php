 ...
 *
 * Okvir se prikazuje ODMAH ISPOD gumba "Dodaj u košaricu" i nudi 4x Boxeri Verzi
	...
	acf_add_local_field_group( array(
		...
		'fields' => array(
			array(
				...
				'label'        => 'Afișează upsell sub buton (4x Boxeri Verzi)',
			...
			array(
				...
				'label'        => 'Afișează upsell #3 sub buton (tricou de compresie NORIKS FIT)',
				'name'         => 'noriks_pp_upsell3',
				'type'         => 'true_false',
				'instructions' => 'Al treilea cadru upsell: tricou de compresie NORIKS FIT, clientul alege mărimea. Independent de primele două comutatoare.',
				'ui'           => 1,
			),
		),
		'location'   => array(
...
function noriks_pp_upsell_config() {
	return apply_filters( 'noriks_pp_upsell_config', array(
		...
		'total'      => 99.99,                   // ista cijena kao thank you upsell (4 komada)
		'title'      => '4x Boxeri Verzi',
		'desc'       => 'Respirabili și moi — adaugă-i la comandă cu %s%% reducere.', // %s = izracunati popust
		'size_attr'  => 'Mărime',
...
function noriks_pp_upsell_render() {
	...
	?>
	<div class="npu-wrap">
		<span class="npu-label">Cumpără împreună și economisește:</span>
			...
			<div class="npu-grid">
				...
				<div class="npu-actions">
					<label class="npu-check">
						...
						<span class="npu-check-text">Adaugă la comandă</span>
					</label>

					<select class="npu-size" name="noriks_pp_upsell_size" aria-label="Mărime boxeri">
...
function noriks_pp_upsell2_config() {
	return apply_filters( 'noriks_pp_upsell2_config', array(
		...
		'label'      => 'Cumpără împreună și economisește:',
		'add_text'   => 'Adaugă la comandă',
		'aria'       => 'Mărime tricouri',
	) );
}
...
function noriks_pp_upsell2_maybe_add( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
	...
	$lines  = array(
		( $prod_a ? $prod_a->get_name() : 'Tricou negru' ) . ' - ' . $size,
		( $prod_b ? $prod_b->get_name() : 'gri' ) . ' - ' . $size,
	);
...
function noriks_pp_upsell3_config() {
	return apply_filters( 'noriks_pp_upsell3_config', array(
		'product_id' => 8354,   // NORIKS FIT kompresijska majica (jednostavan proizvod)
		'total'      => 89.99,  // cijena za 1 komad
		'title'      => 'Tricou de compresie NORIKS FIT',
		'desc'       => 'Modelează silueta și susține postura — adaugă-l la comandă cu %s%% reducere.',
		'size_attr'  => 'Mărime',
...
function noriks_pp_upsell3_order_item_meta( $item, $cart_item_key, $values, $order ) {
	if ( ! empty( $values['_noriks_pp_upsell3'] ) ) {
		...
		if ( ! empty( $values['_noriks_pp_upsell_size'] ) ) {
			$item->update_meta_data( 'Mărime', sanitize_text_field( $values['_noriks_pp_upsell_size'] ) );
		}
	}
}
