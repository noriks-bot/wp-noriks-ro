...
function noriks_remove_upsell() {
    ...
    if ( $order->get_status() !== 'primary-hold' ) {
        wp_send_json_error( 'Timpul pentru modificări a expirat' );
    }
...
function noriks_handle_add_upsell() {
    ...
    if ( $order->get_payment_method() !== 'cod' ) {
        wp_send_json_error( 'Upsell disponibil doar la plata ramburs' );
    }
    if ( $order->get_status() !== 'primary-hold' ) {
        wp_send_json_error( 'Timpul pentru adăugare a expirat' );
    }
    ...
    foreach ( $order->get_items() as $item ) {
        ...
        if ( $item_product_id == $check_product_id || ( $variation_id && $item_variation_id == $variation_id ) ) {
            if ( $item->get_meta( '_noriks_upsell' ) ) {
                wp_send_json_error( 'Ați adăugat deja acest produs' );
            }
        }
    }
    ...
    if ( ! $active_price ) {
        wp_send_json_error( 'Prețul produsului nu este disponibil' );
    }
    ...
    $is_majice = strpos(strtolower(($variation_id && $product_id) ? wc_get_product($product_id)->get_name() : $product->get_name()), 'tricou') !== false || strpos(strtolower(($variation_id && $product_id) ? wc_get_product($product_id)->get_name() : $product->get_name()), 'shirt') !== false;
    ...
    if ( ! $item_id ) wp_send_json_error( 'Eroare la adăugare' );
