...
<div class="nhs-usp">
  ...
  <div class="nhs-usp-grid">
    ...
    <div><span class="nhs-usp-ic">♡</span> Estompează cicatricile peste noapte</div>
    <div><span class="nhs-usp-ic">✚</span> Adeziv suplimentar</div>
    <div><span class="nhs-usp-ic">✓</span> Reutilizabil și durabil</div>
    <div><span class="nhs-usp-ic">↗</span> Stimulează producția de colagen</div>
  </div>
</div>
<style>
...
<!-- KidsNest: pachet summary dupa originalul tryneedo (albastru #2b3fb0, rosu #c0452f) -->
<div class="kn2-badge">
  ...
  <span>Recomandat de pediatri</span>
</div>
<div class="kn2-below">
  <div class="kn2-trust">
    <span><svg width="16" height="16" viewBox="0 0 24 24"><path d="M12 2l8 3v6c0 5-3.5 9.5-8 11-4.5-1.5-8-6-8-11V5l8-3z" fill="#2b3fb0"/><path d="M8.5 12l2.5 2.5 4.5-4.5" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> 90 de nopți de probă</span>
    <span><svg width="18" height="14" viewBox="0 0 24 18"><rect x="1" y="3" width="13" height="10" rx="1.5" fill="#2b3fb0"/><path d="M14 6h4l4 4v3h-8z" fill="#2b3fb0"/><circle cx="6" cy="15" r="2.4" fill="#1b2450"/><circle cx="18" cy="15" r="2.4" fill="#1b2450"/></svg> Livrare rapidă</span>
    <span><svg width="17" height="15" viewBox="0 0 24 20"><rect x="3" y="2" width="18" height="11" rx="2" fill="none" stroke="#2b3fb0" stroke-width="2"/><circle cx="12" cy="7.5" r="2.6" fill="#2b3fb0"/><path d="M2 17c3 2 7 3 10 3s7-1 10-3" fill="none" stroke="#2b3fb0" stroke-width="2" stroke-linecap="round"/></svg> Plată și la livrare (ramburs)</span>
  </div>
  <div class="kn2-redbox">
    <p class="kn2-red-title"><svg width="14" height="13" viewBox="0 0 24 22" style="vertical-align:-2px;margin-right:6px;"><path d="M12 1L23 21H1L12 1z" fill="none" stroke="#c0452f" stroke-width="2.4" stroke-linejoin="round"/><path d="M12 8v6" stroke="#c0452f" stroke-width="2.4" stroke-linecap="round"/><circle cx="12" cy="17.4" r="1.3" fill="#c0452f"/></svg>PENTRU PĂRINȚII COPIILOR SUB 9 ANI</p>
    <p class="kn2-red-body">În fiecare noapte pe o pernă nepotrivită, <strong class="kn2-red">fața copilului dumneavoastră se modelează — în direcția greșită.</strong> Salivare. Sforăit. Gură deschisă. Până la <strong class="kn2-red">9 ani</strong> schimbările se fixează. Până la <strong class="kn2-red">14</strong> — un aparat dentar scump. Până la <strong class="kn2-red">18</strong> — discuția despre intervenții mai serioase. Stomatologii pediatri specializați în căile respiratorii văd asta zilnic — au încetat să o mai numească rară. <strong class="kn2-red">Schimbați perna chiar în seara asta.</strong></p>
  </div>
</div>
<style>
  ...
  /* Ascunde vechiul rand de iconite (features2) — continutul e in randul trust */
  .features2 { display: none !important; }
  /* Spatiu intre titlu si descrierea scurta */
  ...
  /* Checklist (descriere scurta): FARA buline, emoji din continut e iconita, hanging indent */
  ...
  /* chip pret/buc: ascuns */
  .bundle-option .gck-per-chip { display: none !important; }
  /* Dropdown marime: ca originalul (alb, contur gri, fara portocaliu) */
  ...
  /* ascunde % chip, "Total:", countdown si nota */
  ...
  /* ascunde linkul separat "Tabel de marimi" (tabelul e in accordion) */
  .noriks-global-sizechart, #open-size-chart { display: none !important; }
  /* badge-urile implicite ale pluginului -> ribbon albastru sus-dreapta */
...
(function(){
  function init(){
    ...
    /* Ribbon-uri: scoate focul + adauga "Cea mai bună valoare" pe cardul 3 */
    document.querySelectorAll('.gck-popular-badge, .gck-popular-badge-2').forEach(function(b){ b.textContent=b.textContent.replace('🔥','').trim(); if(/Cel mai popular/i.test(b.textContent)) b.textContent='Cel mai popular'; });
    ...
    if(cards.length>=3 && !cards[2].querySelector('.kn2-ribbon') && !cards[2].querySelector('.gck-popular-badge, .gck-popular-badge-2')){
      var rb=document.createElement('div'); rb.className='kn2-ribbon'; rb.textContent='Cea mai bună valoare'; cards[2].appendChild(rb);
    }
    /* Iconite + shipping-box mutate SUB accordion */
    ...
    /* Blocul (trust + redbox + accordioane) sub ADD TO CART */
    ...
    /* Card activ albastru (supravietuieste cache-ului) */
    function paint(){
      if(!sel) return;
...
<!-- ErgoSit: pilula verde "recomandare" (JS o muta imediat sub titlu) + checklist in stilul originalului -->
<div class="oj-recpill">
  <img src="<?php echo esc_url( get_template_directory_uri().'/img/ortopedski-jastuk/04_lijecnik_HR.png' ); ?>" alt="Medic" class="oj-recpill-av" onerror="this.style.display='none'">
  <span><strong>Recomandat de dr. Marić</strong>&nbsp;|&nbsp;<em>Specialist în afecțiuni ale coloanei</em></span>
...
<!-- ErgoSit: pilula mica "stoc" ca in original (in locul cutiei mari rosii de countdown) -->
<div class="oj-stockpill">
  ...
  <em>Au mai rămas doar câteva bucăți!</em>
</div>
<!-- ErgoSit: randul "comenzi in 24h" ca in original (cerc verde pulsand), JS il pune deasupra ADD TO CART -->
<div class="oj-orders">
  <span class="oj-orders-dot"></span>
  <em><strong>128 de comenzi</strong> în ultimele <strong>24 de ore!</strong></em>
</div>
<style>
  ...
  /* Sakrij SAMO riječ "Total:" u ponudama (cijene ostaju) */
  .bundle-total-line > span[style*="font-weight:normal"] { display: none !important; }
  ...
  /* "comenzi in 24h" red (zeleni pulsirajući krug) */
...
(function(){
  ...
  function movePills(){
    ...
    /* "comenzi in 24h" iznad ADD TO CART gumba */
...
<div class="ncl-trust">
  <span>Pentru sticlă și cabine de duș impecabile,</span>
  <span class="ncl-trust-stars">★★★★★</span>
  <span>ne acordă încredere peste 138.452 de gospodării</span>
</div>
<style>
