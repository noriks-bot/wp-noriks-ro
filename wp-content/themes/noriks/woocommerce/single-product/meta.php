    ...
    <div class="accordion-item">
      ...
      <div class="accordion-content">
         ...
         <?php elseif( function_exists('noriks_is_type') && noriks_is_type('kneeheat', $current_product_id) ): ?>
                ...
                <strong>NORIKS KneeHeat</strong> este o manșetă pentru genunchi care îmbină trei terapii într-o singură ședință: căldură de până la 42 °C, compresie cu aer și masaj prin vibrații de 60 Hz. Căldura dilată vasele de sânge și facilitează fluxul de sânge către țesutul mai profund, compresia ritmică împinge lichidul acumulat, iar vibrațiile eliberează rigiditatea din jurul articulației.<br><br>Ședința durează 12 minute și pornește cu un singur buton — fără aplicație, fără setări și fără cablu. Aveți la dispoziție 3 niveluri de căldură și 3 moduri de vibrație, așa că adaptați intensitatea la ziua și la simptomele dumneavoastră.<br><br>Manșeta are o formă ergonomică și se fixează cu benzi reglabile; se potrivește la genunchiul stâng și la cel drept, iar în pachet se află și o bandă de prelungire pentru circumferințe mai mari. Materialul de lângă piele este moale și respirabil.<br><br>Dispozitivul se încarcă prin cablul USB-C inclus, iar o încărcare ajunge pentru mai multe ședințe. Primiți 2 ani garanție de înlocuire.
         ...
         <?php elseif( function_exists('noriks_is_type') && noriks_is_type('controlpro', $current_product_id) ): ?>
                ...
                NORIKS ControlPro este un antrenor mecanic pentru planșeul pelvin, cu rezistență fizică reală. În loc să strângeți în gol, primiți ceva împotriva căruia puteți împinge — același principiu al încărcării progresive prin care se întărește orice alt mușchi din corp.<br><br>Se folosește șezând: aparatul se așază între genunchi și se strânge împotriva rezistenței — 3 serii a câte 10 repetări pe zi, aproximativ cinci minute. Fără sonde și fără inserție, fără cabluri, gel, baterii sau aplicații. Contorul integrat de repetări arată cât ați lucrat.<br><br>Construcția este solidă, cu perne moi pe suporturi, care nu apasă nici după mai multe serii. Rezistența crește progresiv pe măsură ce mușchii se întăresc.<br><br>Este destinat bărbaților care vor să recâștige controlul vezicii — după operația de prostată, la scurgeri în timpul efortului sau după luni lungi de exerciții Kegel fără rezultat. Livrarea este discretă, în cutie neutră, fără însemne.
     ...
     <div class="accordion-item">
      ...
      <div class="accordion-content">
        ...
        <?php elseif( $is_boxers ): ?>
          ...
          <img class="js-open-size-chart" style="cursor:pointer;" src="/ro/wp-content/uploads/2025/12/boxers_size.jpg">
...
<img class="js-open-size-chart" style="cursor:pointer;" src="https://noriks.com/ro/wp-content/uploads/2026/04/bokserice_ro.jpg">
    ...
    <?php if ( ! ( function_exists('noriks_is_type') && ( noriks_is_type('ortopas', $current_product_id) || noriks_is_type('bunion', $current_product_id) || noriks_is_type('fisiorest', $current_product_id) || noriks_is_type('norikshers', $current_product_id) || noriks_is_type('kidsnest', $current_product_id) || noriks_is_type('ortopedski-jastuk', $current_product_id) ) )  && ! ( function_exists('noriks_is_type') && noriks_is_type('kneefix', $current_product_id) || noriks_is_type('controlpro', $current_product_id) )) : // fără sfaturi de spălare pentru centură/bunion/fisiorest/norikshers/kidsnest/perna ortopedică ?>
    <div class="accordion-item">
      <div class="accordion-header" onclick="toggleAccordion(this)">
...
<p>Trimiteți-ne un e-mail în care să spuneți că doriți un înlocuitor și <b data-stringify-type="bold">ne vom ocupa imediat de asta.</b></p>
    ...
    $noriks_manuals = array(
        'kidsnest' => array( 'noriks-kidsnest.pdf', 'NORIKS KidsNest — pernă ortopedică pentru copii' ),
        'kneefix' => array( 'noriks-kneefix.pdf', 'NORIKS KneeFix — orteză ortopedică pentru genunchi' ),
        'ortopedski-jastuk' => array( 'noriks-ergosit.pdf', 'NORIKS ErgoSit — pernă ortopedică de ședere' ),
        'fisiorest' => array( 'noriks-fisiorest.pdf', 'NORIKS FisioRest — dispozitiv pentru gât' ),
        'bunion' => array( 'noriks-bunion-fix.pdf', 'NORIKS Bunion Fix — corector pentru monturi' ),
        'ortopas' => array( 'noriks-ortopas.pdf', 'Centură ortopedică NORIKS pentru spate' ),
    );
    $noriks_manual = null;
      ...
      <div class="accordion-content">
        <a class="noriks-manual-link" href="<?php echo esc_url( get_template_directory_uri() . '/manuals/' . $noriks_manual[0] ); ?>" target="_blank" rel="noopener">
          ...
          <span class="noriks-manual-txt">
            ...
            <small>PDF · se deschide într-o filă nouă</small>
          </span>
